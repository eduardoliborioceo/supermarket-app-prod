...
fun ProdutosScreen(onVoltar: () -> Unit, viewModel: ProdutosViewModel = hiltViewModel()) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Produtos (${state.totalProdutos})") },
                navigationIcon = {
                ...
                actions = {
                    IconButton(onClick = { confirmarRestaurarAberto = true }, enabled = !state.restaurandoPadrao) {
                        Icon(Icons.Filled.Refresh, contentDescription = "Restaurar padrão")
                    }
                },
            )
        },
                            ...
                            CategoriaAdminSection(
                                ...
                                onSalvar = { id, nome, preco, setor ->
                                    viewModel.salvarProduto(id, nome, preco, setor) { resultado ->
                                        if (resultado is ResultadoOperacao.Erro) mensagem = resultado.mensagem
                                        else mensagem = "Produto salvo!"
                                    }
                                },
    ...
    Column(modifier = Modifier.animateContentSize()) {
        Row(
            ...
            Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.weight(1f)) {
                ...
                Text(
                    categoria.produtos.size.toString(),
...
private fun ProdutoAdminCard(
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Text("#${produto.id}", style = MaterialTheme.typography.labelSmall, color = SupermercadoColorTokens.onSurfaceMuted)
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                Button(onClick = { onSalvar(nome.trim(), parseMoeda(precoTexto), setor) }, modifier = Modifier.weight(1f)) {
                    Text("Salvar")
                }
                OutlinedButton(
                    onClick = onExcluir,
                    colors = androidx.compose.material3.ButtonDefaults.outlinedButtonColors(contentColor = SupermercadoColorTokens.error),
                ) {
